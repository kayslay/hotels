...
/**
 *  @description removes duplicate hotels frm the array
...
(async function() {
	...
	const updateConfig = {
		...
		fetchFn: function (err, data, url) {
			...
			if (updateArr.length == 100){
				console.log("copying next 100")
				const copy = updateArr.splice(0)
				copy.forEach(data => {
					hotelModel.update({
						hotelsNg_link: data[1]
					}, data[0])
						.then(data => {
							console.log("saved to db from break")
						})
				})
			}
	...
	if (process.env.UPDATE =="yep") crawler(updateConfig).CrawlAllUrl();
})

crawler(config).CrawlAllUrl()